...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        att = torch.sigmoid(self.mlp(avg) + self.mlp(max_pool))
        return x * att
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        att = torch.sigmoid(self.conv(torch.cat([avg, max_pool], dim=1)))
        return x * att


class AttentionFusion(nn.Module):
    """
    Channel attention and spatial attention are applied in cascade, then the
    attended feature is appended to the original feature and fused by 1x1 conv.
    """
    ...
    def __init__(self, ch):
        ...
        self.fuse = ConvBNReLU(ch * 2, ch, k=1, padding=0)

    def forward(self, x):
        att = self.channel(x)
        att = self.spatial(att)
        return self.fuse(torch.cat([x, att], dim=1))


class UpBlock(nn.Module):
    def __init__(self, in_ch, skip_ch, out_ch, dropout=0.05):
        super().__init__()
        self.skip_att = AttentionFusion(skip_ch)
        self.conv = ConvBlock(in_ch + skip_ch, out_ch, dropout)

    def forward(self, x, skip):
        x = F.interpolate(x, size=skip.shape[-2:], mode="bilinear", align_corners=False)
        skip = self.skip_att(skip)
        return self.conv(torch.cat([x, skip], dim=1))


class AutoDetectionAttentionNet(nn.Module):
    """
    Attention-based ocean eddy segmentation baseline.
    It uses Conv-BN-ReLU encoder blocks, channel and spatial attention fusion,
    skip concatenation, and a decoder that restores full-map logits.
    """
    ...
    def __init__(self, in_ch=1, num_classes=3, base=32, dropout=0.05):
        ...
        self.att1 = AttentionFusion(b)
        self.enc2 = ConvBlock(b, b * 2, dropout)
        self.att2 = AttentionFusion(b * 2)
        self.enc3 = ConvBlock(b * 2, b * 4, dropout)
        self.att3 = AttentionFusion(b * 4)
        self.enc4 = ConvBlock(b * 4, b * 8, dropout)
        self.att4 = AttentionFusion(b * 8)

        self.center = ConvBlock(b * 8, b * 16, dropout)
        self.center_att = AttentionFusion(b * 16)

        self.up4 = UpBlock(b * 16, b * 8, b * 8, dropout)
        self.up3 = UpBlock(b * 8, b * 4, b * 4, dropout)
        self.up2 = UpBlock(b * 4, b * 2, b * 2, dropout)
        self.up1 = UpBlock(b * 2, b, b, dropout)

        self.out = nn.Conv2d(b, num_classes, 1)

    def forward(self, x):
        e1 = self.att1(self.enc1(x))
        e2 = self.att2(self.enc2(F.max_pool2d(e1, 2)))
        e3 = self.att3(self.enc3(F.max_pool2d(e2, 2)))
        e4 = self.att4(self.enc4(F.max_pool2d(e3, 2)))

        c = self.center_att(self.center(F.max_pool2d(e4, 2)))
...
def main():
    ...
    parser.add_argument(
        "--out_dir",
        type=str,
        default="runs/AutoDetectionAttention/scse_fullmap_base32_e80",
    )
    ...
    model = AutoDetectionAttentionNet(base=args.base, dropout=args.dropout).to(device)
