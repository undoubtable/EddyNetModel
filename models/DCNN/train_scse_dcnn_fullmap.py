...
class DCNN(nn.Module):
    """
    Shallow fully-convolutional encoder-decoder baseline for segmentation.
    It intentionally avoids U-Net dense skips, pyramid pooling, and ASPP.
    """
    ...
    def __init__(self, in_ch=1, num_classes=3, base=32, dropout=0.05):
        ...
        self.down1 = nn.Sequential(
            ConvBNReLU(b, b * 2, stride=2),
            ConvBlock(b * 2, b * 2, dropout),
        )
        self.down2 = nn.Sequential(
            ConvBNReLU(b * 2, b * 4, stride=2),
            ConvBlock(b * 4, b * 4, dropout),
        )
        self.bottleneck = ConvBlock(b * 4, b * 4, dropout)

        self.up2 = ConvBlock(b * 4, b * 2, dropout)
        self.up1 = ConvBlock(b * 2, b, dropout)
        self.refine = ConvBlock(b, b, dropout)
...
def main():
    ...
    parser.add_argument("--out_dir", type=str, default="runs/DCNN/scse_fullmap_base32_e80")
    ...
    model = DCNN(base=args.base, dropout=args.dropout).to(device)
