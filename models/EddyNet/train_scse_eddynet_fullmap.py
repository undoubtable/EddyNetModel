...
class ConvBNReLU(nn.Module):
    def __init__(self, in_ch, out_ch, k=3, padding=1):
        super().__init__()
        self.conv = nn.Conv2d(in_ch, out_ch, k, padding=padding, bias=False)
...
class EddyNet(nn.Module):
    """
    EddyNet-style U-Net encoder-decoder.
    The original EddyNet code uses repeated Conv-BN-ReLU blocks, max-pooling,
    nearest upsampling, and skip concatenation. This baseline keeps that shape
    with a configurable base channel count for SCSE full-map comparison.
    """
    ...
    def __init__(self, in_ch=1, num_classes=3, base=32, dropout=0.05):
        ...
        self.enc1 = EddyBlock(in_ch, b, dropout)
        self.enc2 = EddyBlock(b, b, dropout)
        self.enc3 = EddyBlock(b, b, dropout)

        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)

        self.center = EddyBlock(b, b, dropout)

        self.dec3 = EddyBlock(b * 2, b, dropout)
        self.dec2 = EddyBlock(b * 2, b, dropout)
        self.dec1 = EddyBlock(b * 2, b, dropout)

        self.out = nn.Conv2d(b, num_classes, kernel_size=1, padding=0, bias=False)
        nn.init.kaiming_normal_(self.out.weight, mode="fan_out", nonlinearity="linear")
    ...
    def forward(self, x):
        ...
        u3 = F.interpolate(center, size=e3.shape[-2:], mode="nearest")
        d3 = self.dec3(torch.cat([u3, e3], dim=1))

        u2 = F.interpolate(d3, size=e2.shape[-2:], mode="nearest")
...
def main():
    ...
    parser.add_argument("--out_dir", type=str, default="runs/EddyNet/scse_fullmap_base32_e80")
    ...
    model = EddyNet(base=args.base, dropout=args.dropout).to(device)
